...
# ── ADB config ─────────────────────────────────────────────────────────────────
ADB_HOST = "192.168.29.212:5555"
ADB_PATH = r"C:\Users\itsat\OneDrive\Desktop\platform-tools-latest-windows\platform-tools\adb.exe"

KEYEVENTS = {"LEFT": 21, "RIGHT": 22, "UP": 19, "DOWN": 20, "CENTER": 23, "BACK": 4}
...
# ── Fist / BACK tuning ────────────────────────────────────────────────────────
FIST_COOLDOWN_S = 1.0         # seconds before fist can fire again
...
class GestureState:
    def __init__(self) -> None:
        ...
        self.last_thumbsup_time: float = 0.0
    ...
    def check_open_palm(self, lms) -> bool:
        """Fires when all five fingers including thumb are fully extended."""
        if time.time() - self.last_fist_time < FIST_COOLDOWN_S:
            return False
        # All four fingers extended: tip above PIP joint
        finger_pairs = [(8, 6), (12, 10), (16, 14), (20, 18)]
        for tip_i, pip_i in finger_pairs:
            if lms[tip_i].y > lms[pip_i].y:  # tip is below PIP = finger curled
                return False
    # Thumb extended: tip above thumb IP joint (landmark 3)
        if lms[4].y > lms[3].y:
            return False
...
def draw_hud(frame, state: "GestureState", last_gesture: str | None, cooldown_left: float,
    ...
    # Fist indicator
    fist_cooldown = max(0.0, FIST_COOLDOWN_S - (time.time() - state.last_fist_time))
    fist_ready = fist_cooldown == 0
    fist_col = (0, 220, 80) if fist_ready else (60, 60, 60)
    cv2.putText(frame, "Closed fist = BACK", (bx, by + 30), FONT, 0.48, (160, 160, 160), 1, cv2.LINE_AA)
    label_fist = "READY" if fist_ready else f"cooldown {fist_cooldown:.1f}s"
    cv2.putText(frame, label_fist, (bx, by + 46), FONT, 0.48, fist_col, 1, cv2.LINE_AA)

    # Instruction hint (bottom-right)
    hints = ["Point index → swipe", "Thumbs up → OK", "Closed fist → BACK"]
    for i, hint in enumerate(hints):
        cv2.putText(frame, hint, (w - 210, h - 56 + i * 20),
...
def main() -> None:
    ...
    if ADB_HOST:
        ...
        r = subprocess.run([ADB_PATH, "connect", ADB_HOST], capture_output=True, text=True)
    ...
    with HandLandmarker.create_from_options(options) as landmarker:
        while True:
            ...
            if result.hand_landmarks:
                ...
                # Fist check first, then thumbs-up, then swipe
                if state.check_open_palm(lms):
                    gesture = "BACK"
                elif state.check_thumbs_up(lms):
                    gesture = "CENTER"
                elif index_extended:
